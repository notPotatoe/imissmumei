...
{  
  ...
  const options = {
    method: 'POST',
    headers: {'Content-Type': 'application/json', 'X-APIKEY': 'YOUR-HOLODEX-API'} ,
    body: '{"sort":"newest", "lang":["en","ja"],"target":["stream"],"conditions":[],"vch":["'+mumei_id+'"],"org":["Hololive"],"comment":[],"paginated":true,"offset":0,"limit":5}'
  };
  ...
  if(is_live)
  {
    ...
    //so this checks if the NEXT item on the list is actually her
    //probably would need some more work but it works for now
        is_mumei = false;
      if (data.items[stream_id+1].channel.id == mumei_id && data.items[stream_id+1].status == "live" )
      {
        is_mumei = true;
        stream_id++;
      }
  ...
  stream_id = 0;
  while (!is_past && stream_id < 5)
  {
    if (data.items[stream_id].status != "upcoming")
      is_past = true;
    else
        stream_id++;
  }
...
function live(channel_id, title, vid_id, is_mumei)
{
  ...
  fetch('https://www.googleapis.com/youtube/v3/activities?part=snippet,contentDetails&maxResults=2&channelId='+channel_id+'&type=upload&key=YOUR-YOUTUBE-API-KEY')
    .then(response => response.json())
    .then(data => {
      //console.log(data);
